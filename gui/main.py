from tkinter import Tk,Button,messagebox,PhotoImage,Label,Frame,Entry,DISABLED,NORMAL,END,StringVar,Radiobutton
...
class RegWindow():
    ...
    def __init__(self, root, title, size, IP):
        ...
        self.logbackButton = Button(self.frame,text = "Login",width=10,font=("yu gothic ui", 18, "bold"),cursor = "hand2",command= self.logback).place(x=530,y=530)

    def logback(self):
        self.root.destroy()  
        loginWindow = Tk() 
        obj = LoginWindow(loginWindow,"Face Recognition", "1166x718",self.ip)   
        loginWindow.mainloop()
